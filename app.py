...
def getAdminLoginDetails():
    with sqlite3.connect('database.db') as conn:
        ...
        if 'email' not in session:
            ...
            loggedIn = True
            cur.execute("SELECT firstName FROM admindetails WHERE email = '" + session['email'] + "'")
...
def adminPanel():
    ...
    loggedIn, firstName = getAdminLoginDetails()
    if (success_code == "200"):
        with sqlite3.connect('database.db') as conn:
            cur = conn.cursor()
            cur.execute('SELECT productId, name, price, description, image, stock FROM products')
            ...
            cur.execute('SELECT categoryId, name FROM categories')
            categoryData = cur.fetchall()
        return render_template('adminpanel.html', itemData=itemData, categoryData=categoryData,  loggedIn = loggedIn, firstName = firstName)
    else:
...
def orders():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT products.productId, products.name, products.price, products.image, orders.qty, orders.totalprice, orders.purchasedate, orders.purchasetime FROM products, orders WHERE products.productId = orders.productId AND orders.userId = " + str(userId))
...
def searchItem():   
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        return render_template('result.html', products=products, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems)  
...
def displayCategory():
        ...
        with sqlite3.connect('database.db') as conn:
            cur = conn.cursor()
            cur.execute("SELECT products.productId, products.name, products.price, products.description, products.image, categories.name FROM products, categories WHERE products.categoryId = categories.categoryId AND categories.categoryId = " + categoryId)
            products = cur.fetchall()
            cur.execute('SELECT categoryId, name FROM categories')
            categoryData = cur.fetchall()
        conn.close()
        categoryName = products[0][4]
        # products = parse(products)
        return render_template('displayCategory.html', products=products, categoryData=categoryData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryName=categoryName)
...
def checkout():
    ...
    productPurchaseDate = x.strftime("%d-%m-%Y")
    productPurchaseTime = x.strftime("%H:%M%p")
    # print(productPurchaseDate, productPurchaseTime)
    email = session['email']
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute("SELECT userId FROM users WHERE email = '" + email + "'")
